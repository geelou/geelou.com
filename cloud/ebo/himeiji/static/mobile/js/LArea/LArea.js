/**
 * 设置select的option选项内容
 * @param tmpArray 原始数据数组
 * @param pid 
 * @param typename select类型
 * @param defaultValue 默认值
 * @returns
 */
function pc_setOption(tmpArray,pid,typename,defaultValue,needNullOption){
	var subArray = [];	
	$('#'+pid+ ' .'+typename).html('');
	
	if(needNullOption){
		$('#'+pid+ ' .'+typename).append("<option value=''>"+$('#'+pid+ ' .'+typename).attr('data-title')+"</option>");					
	}
	
	if(tmpArray && tmpArray.length>0){
		subArray=tmpArray[0].children;
		for(var i = 0;i<tmpArray.length;i++){ 
			if(defaultValue && tmpArray[i].text == defaultValue){
				$('#'+pid+ ' .'+typename).append("<option value='"+tmpArray[i].value+"' selected='selected'>"+tmpArray[i].text+"</option>");			
				subArray = tmpArray[i].children;
			}else{
				$('#'+pid+ ' .'+typename).append("<option value='"+tmpArray[i].value+"'>"+tmpArray[i].text+"</option>");
			}
		}
	}
	return subArray;
...
function pc_initPcCity(pid){
	var provinceValue = $('#'+pid+ ' .province').attr('data-value');
	var cityValue = $('#'+pid+ ' .city').attr('data-value');
	var areaValue = $('#'+pid+ ' .area').attr('data-value');
	
	var needNullOption = true;
	if(provinceValue){
		needNullOption=false;
	}
	var subArray = pc_setOption(cityData3,pid,'province',provinceValue,needNullOption);
	if(needNullOption){
		pc_setOption([],pid,'city',cityValue,needNullOption);
		pc_setOption([],pid,'area',areaValue,needNullOption);
	}else{
		subArray = pc_setOption(subArray,pid,'city',cityValue,needNullOption);
		pc_setOption(subArray,pid,'area',areaValue,needNullOption);
	}
	
	
	pc_setCityPickerText(pid);
}

function pc_changeOption(obj){
	var curObj = $(obj);
	var selectValue = curObj.val(); // 选中的编号
	var pid = curObj.parent().attr("id");
	
	var subClass = curObj.attr("data-subclass"); // 下级选择框
	var tmpArray = [];
	var needNullOption = true;
	if(selectValue){
		needNullOption=false;
	}
	if(subClass){ // 如果有下级选择框
		tmpArray = pc_findChildArray(cityData3,selectValue);
		tmpArray = pc_setOption(tmpArray,pid,subClass,'',needNullOption);
	}
	if($('#'+pid + ' .'+subClass).attr('data-subclass')){ // 如果有下一级
		pc_setOption(tmpArray,pid,$('#'+pid + ' .'+subClass).attr('data-subclass'),'',needNullOption);
	}	
	pc_setCityPickerText(pid);
}

function pc_setCityPickerText(pid){
	var inputId = $('#'+pid).attr('data-input_id');
	var pvalue = $('#'+pid + ' .province').val();
	var ptext = $('#'+pid + ' .province')[0].options[$('#'+pid + ' .province')[0].selectedIndex].text;
	var cvalue = $('#'+pid + ' .city').val();
	var ctext = $('#'+pid + ' .city')[0].options[$('#'+pid + ' .city')[0].selectedIndex].text;
	var avalue = $('#'+pid + ' .area').val();
	var atext = $('#'+pid + ' .area')[0].options[$('#'+pid + ' .area')[0].selectedIndex].text;
	
	var cityPickerText = '';
	if(pvalue){
		cityPickerText += ptext + ' ';
	}
	if(cvalue){
		cityPickerText += ctext + ' ';
	}
	if(avalue){
		cityPickerText += atext + ' ';
	}
	$('#'+inputId).val(cityPickerText);
	//alert($('#'+inputId).val());
}

function pc_findChildArray(tmpArray,value){
	for(var i = 0;i<tmpArray.length;i++){ 
		if(tmpArray[i].value == value){
			return tmpArray[i].children;
		}
		if(tmpArray[i].children){
			var result = pc_findChildArray(tmpArray[i].children,value);
			if(result){
				return result;
			}
		}
...
window.LArea = (function() {
    ...
    MobileArea.prototype = {
        init: function(params) {
            ...
            if(params.keys){this.keys = params.keys;}
            if(params.childKey){this.childKey = params.childKey;}
        ...
        setGearTooth: function(data) {
            ...
            if (l > 0) {
                ...
                switch (_self.type) {
                    case 1:
                    childData = item[gearVal][_self.childKey];
                        break;
                    case 2:
        ...
        finish: function(e) {
            ...
            _self.trigger.value = provinceText + ((cityText)?(_self.splitChar + cityText):(''))+ ((countyText)?(_self.splitChar + countyText):(''));
            _self.value = [provinceVal, cityVal, countyVal];
            if(this.valueTo){
                this.valueTo.value= provinceCode +((cityCode)?(_self.splitChar + cityCode):('')) + ((countyCode)?(_self.splitChar + countyCode):(''));
            }
            _self.close(e);
        },
        close: function(e) {
